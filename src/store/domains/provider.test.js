...
it('handles providerSet event properly', () => {
  ...
  const provider = getModel([eventCreators.providerSet(options)]);
...
it('handles providerRequested event properly', () => {
  const provider = getModel([eventCreators.providerRequested()]);

  expect(provider.isLoading()).toEqual(true);
});

it('handles providerError event properly', () => {
  const provider = getModel([eventCreators.providerError('Could not find provider')]);
